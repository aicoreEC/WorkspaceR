...
#                             입력 오류나 실제 특이값일수도 있다.
#                             특이값의 성질은 제조공정의 불량품 선별, 은행 거래 시스템의 
#                             사기 거래 탐지할 때 사용하기도 함
#
# 데이터 분석시 특이값을 포함한 채 평균 등을 계산하면 전체 데이터 양상 파악에 왜곡을 가져올 수 
#                             있으므로 분석시 제외 경우 많음
#
# 데이터셋에 특이값이 포함되어 있는지 여부 기준
# 1. 논리적으로 있을 수 없는 값이 있는지 찾는다. 특별한 방법은 없기 때문에 분석자가 
#    각 변수의 특성을 이해한 후 특이값 탐색
